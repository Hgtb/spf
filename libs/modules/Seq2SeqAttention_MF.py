...
# import libs.modules.Seq2SeqAttention_MF
from libs.modules.includes import *
from libs.modules.Encoder_AttentionDecoder_MF import *
...
class Seq2SeqAttention(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers, dropout):
        ...
        self.encoder = Seq2SeqEncodeNoEmbedding(input_size=input_size, hidden_size=hidden_size,
                                                num_layers=num_layers, dropout=dropout)
        self.decoder = Seq2SeqAttentionDecoderNoEmbedding(input_size=input_size, hidden_size=hidden_size, output_size=output_size,
                                                          num_layers=num_layers, dropout=dropout)
...
def train_Seq2SeqAttention_MF(model: Seq2SeqAttention,
                              device: torch.device,
                              lr: float,
    ...
    for epoch in range(num_epochs):
        ...
        for X, Y in tqdm_:
            # X : torch.Size([stock_num, enc_steps, parameters_num])
            # Y : torch.Size([stock_num, dec_steps, parameters_num])
            optimizer.zero_grad()
...
def eval_Seq2SeqAttention_MF(model: Seq2SeqAttention,
                             device: torch.device,
                             dataLoader,
                             steps: int,  # dec_steps
    ...
    for model_input, target_data in tqdm(dataLoader):
